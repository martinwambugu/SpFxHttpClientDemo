...
export default class SpFxHttpClientDemoWebPart extends BaseClientSideWebPart<ISpFxHttpClientDemoWebPartProps> {
  ...
  public render(): void {
    const element: React.ReactElement<ISpFxHttpClientDemoProps> =
      React.createElement(SpFxHttpClientDemo, {
  ...
  private _updateListItem(): Promise<SPHttpClientResponse> {
    ...
    return this.context.spHttpClient
      .get(
        this.context.pageContext.web.absoluteUrl +
          `/_api/web/lists/getbytitle('Countries')/items?$select=Id,Title&$filter=Title eq 'Tanzania'`,
        SPHttpClient.configurations.v1,
      )
      .then((response) => {
        return response.json();
  ...
  private _deleteListItem(): Promise<SPHttpClientResponse> {
    ...
    return this.context.spHttpClient
      .get(
        this.context.pageContext.web.absoluteUrl +
          `/_api/web/lists/getbytitle('Countries')/items?$select=Id,Title&$orderby=ID desc&$top=1`,
        SPHttpClient.configurations.v1,
      )
      .then((response) => {
        return response.json();
